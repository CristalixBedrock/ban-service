...
Bans.create = (newTutorial, result) => {
  sql.query("INSERT INTO mutes SET ?", newTutorial, (err, res) => {
    if (err) {
...
Bans.findByNickname = (id, result) => {
  sql.query("SELECT * FROM mutes WHERE player='" + id + "'", (err, res) => {
    if (err) {
      console.log("error: ", err);
      result(err, null);
      return;
...
Bans.getAllActive = result => {
  sql.query("SELECT * FROM mutes WHERE active=true", (err, res) => {
    if (err) {
      console.log("error: ", err);
      result(null, err);
      return;
...
Bans.updateByNickname = (player, tutorial, result) => {
  sql.query(
    "UPDATE mutes SET player = ?, reason = ?, ban_id = ?, banner = ?, active = ?, banned_date = ?, ban_expiration = ? WHERE player = ?",
...
Bans.remove = (player, result) => {
  sql.query("DELETE FROM mutes WHERE player = ?", player, (err, res) => {
    if (err) {
      console.log("error: ", err);
      result(null, err);
      return;
...
Bans.removeAll = result => {
  sql.query("DELETE FROM mutes", (err, res) => {
    if (err) {
      console.log("error: ", err);
      result(null, err);
      return;
