const Bans = require("../models/mutes.model.js");

// Create and Save a new Mute
exports.create = (req, res) => {
  ...
  // Save mute in the database
  Bans.create(tutorial, (err, data) => {
    if (err)
      res.status(500).send({
        message:
          err.message || "Some error occurred while creating the Mute."
      });
    else res.send(data);
  });
};

// Retrieve all Mutes from the database (with condition).
exports.findAll = (req, res) => {
  ...
  Bans.getAll(title, (err, data) => {
    if (err)
      res.status(500).send({
        message:
          err.message || "Some error occurred while retrieving mutes."
      });
    else res.send(data);
  });
};

// Find a single Mute by Id
...
// Update a Mutes identified by the id in the request
...
// Delete a Mute with the specified id in the request
...
// Delete all Mute from the database.
exports.deleteAll = (req, res) => {
  Bans.removeAll((err, data) => {
    ...
    else res.send({ message: `All Players|Mutes were deleted successfully!` });
  });
};
